...
public class IsoSpriteSorting : MonoBehaviour {
    ...
    [SerializeField]
    private SpriteDepthDatabase spriteDepthDatabase;
    ...
    private static int ComparePointAndLine(Vector3 point, IsoSpriteSorting line) {
        ...
        if (pointY > line.SortingPoint1.y && pointY > line.SortingPoint2.y) {
            ...
            float dx = (line.SortingPoint2.x - line.SortingPoint1.x);
            if (Mathf.Abs(dx) < 0.0001f) {
                return point.y > line.SortingPoint1.y ? -1 : 1;
            }
            float slope = (line.SortingPoint2.y - line.SortingPoint1.y) / dx;
    ...
    public int RendererSortingOrder {
        get {
            return (renderersToSort != null && renderersToSort.Length > 0)
                ? renderersToSort[0].sortingOrder
                : 0;
        }
        set {
            if (renderersToSort == null) return;
            for (int i = 0; i < renderersToSort.Length; i++) {
    ...
    private void SetTransform() {
        if (spriteRenderer != null) {
            t = spriteRenderer.gameObject.transform;
        }
        else {
            SetTransform();
        }
